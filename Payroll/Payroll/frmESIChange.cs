...
namespace Payroll
{
    public partial class frmESIChange : Form
    {
        ...
        public DataTable EmpList(string xyz,string abc)
        {
            string sql = "SELECT DISTINCT a.emp_code as 'Code',a.emp_name as 'Emp Name',d.office_type_id as 'Office Type',e.office_name as 'Office Name',f.dept_name as 'Department',g.desgn_name as 'Designation',date_format(a.emp_joining_date, '%d-%m-%Y') as 'Joining Date',b.employment_type_name as 'Employment' FROM `tbl_employment_type` b,`tbl_employee` a,`tbl_employee_employment` c,`tbl_employee_posting`d,`tbl_office` e,`tbl_department` f,`tbl_designation` g,tbl_employee_salary h WHERE h.is_esi_applicable = '"+abc+"' and h.emp_id = a.emp_code and c.emp_id = a.emp_code AND b.employment_type_id = c.employment_type_id  AND d.emp_id = a.emp_code AND d.office_id = e.office_id AND d.dept_id = f.dept_id AND d.desgn_id = g.desgn_id and a.emp_status = '" + xyz + "' ";
        ...
        private void frmESIChange_Load(object sender, EventArgs e)
        {
            esate();
            isESI();
        }
        ...
        {
            dataGridView1.DataSource = EmpList(esate(), isESI());
        }
